...
public class SplitWordCount {
	...
	public static void main(String[] args) {
		...
		terms.parallelStream() // Utilizes multi-core hardware
				...
				.collect(Collectors.toConcurrentMap(w -> w.toLowerCase(), w -> 1, Integer::sum)) // BigO(n) 
	...
	static class TopOccurrence {
		...
		public TopOccurrence(int maxSize) {
			...
			// This constructs a min heap (when order of elements is natural
			// i.e. ascending order).
			// We are using Natural order for integers (wc.count)
			// In order to create a max-heap, we just need to provide reversed
			// comparator i.e. that sorts in descending order,as shown below
			// this.minHeap = new
		...
		public void add(WordCount data) {
			...
			} else if (minHeap.peek().count < data.count) { // peek() is Big
															// O(1)
